...
(function (factory) {
	...
	// imageSRC - DNIT - transformar em imagem para impressao
	Highcharts.imageSrc = function (dataURL, filename, chart) {
		$("#img_" + chart.renderTo.id).html("<img src=\'" + dataURL + "\' style='width: 80%;' />");
	};
	...
	// Get data URL to an image of an SVG and call download on it
	Highcharts.imageSVGLocal = function (chart, svg, filename, imageType, scale, failCallback, successCallback) {
		var svgurl,
			blob,
			objectURLRevoke = true,
			finallyHandler;
		...
		if (imageType === 'image/svg+xml') {
			...
			try {
				...
				Highcharts.imageSrc(svgurl, filename, chart);
				if (successCallback) {
					successCallback();
				}
			} catch (e) {
			...
			Highcharts.imageToDataUrl(svgurl, imageType, { /* args */ }, scale, function (imageURL) {
				// Success
				try {
					Highcharts.imageSrc(imageURL, filename, chart);
					if (successCallback) {
						successCallback();
					}
				} catch (e) {
	 ...
	 * Add a new method to the Chart object to perform a local download
	 */
	Highcharts.Chart.prototype.exportImageLocal = function (exportingOptions, chartOptions) {
		var chart = this,
			options = Highcharts.merge(chart.options.exporting, exportingOptions),
			imageType = options && options.type || 'image/png',
			fallbackToExportServer = function () {
			...
			svgSuccess = function (svg) {
				var filename = (options.filename || 'chart') + '.' + (imageType === 'image/svg+xml' ? 'svg' : imageType.split('/')[1]);
				Highcharts.imageSVGLocal(chart, svg, filename, imageType, options.scale, fallbackToExportServer);
			};
